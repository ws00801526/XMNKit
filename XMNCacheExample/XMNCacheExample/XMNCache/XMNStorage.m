...
static NSString *const kXMNDBFileName = @"manifest.sqlite";
static NSString *const kXMNDBShmFileName = @"manifest.sqlite-shm";
static NSString *const kXMNDBWalFileName = @"manifest.sqlite-wal";
static NSString *const kXMNDataDirectoryName = @"com.XMFraker.cacheData";
static NSString *const kXMNTrashDirectoryName = @"com.XMFraker.cacheTrash";
...
@implementation XMNStorageObject

@end


@implementation XMNStorage
{
    ...
    /** 应用终止后置为YES 如果YES,则不应该在打开数据库,并且忽略所有读写操作 */
    BOOL _invalidated;
    /** 数据库关闭中 */
    BOOL _dbClosing;
    /** 线程锁,保证线程安全 */
...
- (instancetype)init {
    NSAssert(NO, @"you should use \"initWithPath: type:\" to init a XMNStorage");
    return [self initWithPath:nil type:XMNStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(XMNStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSAssert(NO, @"XMNStorage path.length is too long and invalid path [%@] ",path);
    }
    if (type > XMNStorageTypeMixed) {
        NSAssert(NO, @"XMNStorage type is invalid");
    }
    if (self = [super init]) {
        _path = [path copy];
        _type = type;
        _dataPath = [path stringByAppendingPathComponent:kXMNDataDirectoryName];
        _trashPath = [path stringByAppendingPathComponent:kXMNTrashDirectoryName];
        _dbPath = [path stringByAppendingPathComponent:kXMNDBFileName];
        
        _trashQueue = dispatch_queue_create("com.XMFraker.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error] ||
            ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kXMNDataDirectoryName]
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error] ||
            ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kXMNTrashDirectoryName]
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error]) {
                NSAssert(NO, @"XMNStorage init path error :%@",error);
            }
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            ...
            if (![self _dbOpen] || ![self _dbInitialize]) {
                [self _dbClose];
                NSAssert(NO, @"XMNStorage init sqlite error : fail to open sqlite db");
            }
        }
...
- (BOOL)saveObject:(XMNStorageObject *)object {
    return [self saveObjectWithKey:object.key value:object.value filename:object.filename extendedData:object.extendedData];
}

- (BOOL)saveObjectWithKey:(NSString *)key value:(NSData *)value {
    return [self saveObjectWithKey:key value:value filename:nil extendedData:nil];
}

- (BOOL)saveObjectWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    if (key.length == 0 || value.length == 0) return NO;
    if (_type == XMNStorageTypeFile && filename.length == 0) {
        return NO;
    }
...
- (BOOL)removeObjectForKey:(NSString *)key {
    ...
    switch (_type) {
        case XMNStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case XMNStorageTypeFile:
        case XMNStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self fileDeleteWithName:filename];
            }
...
- (BOOL)removeObjectsLargerThanSize:(int)size {
    ...
    if (size <= 0) return [self removeAllObjects];
...
- (BOOL)removeObjectsEarlierThanTime:(NSTimeInterval)time {
    ...
    if (time == INT_MAX) return [self removeAllObjects];
...
- (BOOL)removeObjectsToFitSize:(int)maxSize {
    ...
    if (maxSize <= 0) return [self removeAllObjects];
    ...
    do {
        int perCount = 16;
        objects = [self _dbGetItemSizeInfoOrderByTimeDescWithLimit:perCount];
        for (XMNStorageObject *object in objects) {
            if (total > maxSize) {
                if (object.filename) {
                    [self fileDeleteWithName:object.filename];
                }
                suc = [self _dbDeleteItemWithKey:object.key];
                total -= object.size;
            } else {
                break;
            }
    ...
    } while (total > maxSize && objects.count > 0 && suc);
    return suc;
}
...
- (BOOL)removeObjectsToFitCount:(int)maxCount {
    ...
    if (maxCount <= 0) return [self removeAllObjects];
    ...
    do {
        ...
        for (XMNStorageObject *item in items) {
            if (total > maxCount) {
                if (item.filename) {
...
- (void)removeAllObjectsWithProgressBlock:(void (^)(NSUInteger removedCount, NSUInteger totalCount))progressBlock
    ...
    int total = [self _dbGetTotalItemCount];
    if (total <= 0) {
        if (finishBlock) finishBlock(total < 0);
    } else {
        int left = total;
        int perCount = 32;
        NSArray *items = nil;
        ...
        do {
            ...
            if (progressBlock) progressBlock(total - left, total);
        } while (left > 0 && items.count > 0 && suc);
        if (finishBlock) finishBlock(!suc);
    }
...
- (XMNStorageObject *)getObjectForKey:(NSString *)key {
    ...
    XMNStorageObject *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (XMNStorageObject *)getObjectInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    XMNStorageObject *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSDictionary *)getObjectValuesForKeys:(NSArray *)keys {
    NSMutableArray *items = (NSMutableArray *)[self getObjectsForKeys:keys];
 ...
 */
- (void)reset {
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kXMNDBFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kXMNDataDirectoryName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kXMNTrashDirectoryName] error:nil];
...
- (BOOL)_dbIsReady {
    return (_db && !_dbClosing && !_invalidated);
}
...
- (XMNStorageObject *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    XMNStorageObject *item = [XMNStorageObject new];
...
- (XMNStorageObject *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    XMNStorageObject *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            XMNStorageObject *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
